...
echo "  # then click Send 24-tile job on the dashboard, or:"
echo "  nova run demo/gallery.yaml"
echo "  open the printed http://<lan-ip>:8080"
echo
echo "Two services (job dashboard + GPU worker):"
echo "  nova dashboard"
echo "  NOVA_COORDINATOR_URL=http://<lan-ip>:8080 nova worker"
echo "  ./scripts/install-service.sh dashboard   # systemd/launchd"
echo "  ./scripts/install-service.sh worker"
echo
...
echo "  nova dashboard      # projector / submitter"
echo "  nova worker         # GPU box (NOVA_COORDINATOR_URL=...)"
